...
interface Application {
  ...
  skillsJson: Record<string, string[]> | string[];
  status: string;
  notes: string | null;
...
export default function VolunteerAdminPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const url = filter !== 'all' ? `/api/admin/volunteer?status=${filter}` : '/api/admin/volunteer';
              ...
              >
                <div className="flex items-center gap-4">
                  <div>
                    ...
                    <div className="text-xs text-[#AAB0D6] mt-0.5">{app.email} — {app.roleType}</div>
                  </div>
                </div>
                ...
                <div className="border-t border-white/10 px-5 py-4 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <div className="text-[#F5F3EE]">{app.motivationText}</div>
                    </div>
                    <div>
                      <div className="text-[#AAB0D6] text-xs mb-1">Skills</div>
                      <div className="text-[#F5F3EE] text-xs">{JSON.stringify(app.skillsJson)}</div>
                    </div>
                  </div>
