...
interface Application {
  ...
  phone: string | null;
  motivation: string;
  spiritualExperience: string | null;
...
export default function PathwayAdminPage() {
  ...
  async function updateStatus(id: string, status: string) {
    setSaving(id);
    await fetch('/api/admin/pathway', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, status, reviewerNotes: notes[id] ?? null }),
    });
    await load();
    setSaving(null);
  }
              ...
              {expanded === app.id && (
                <div className="border-t border-white/10 px-5 py-4 space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    ...
                    {app.spiritualExperience && (
                      <div>
                        <div className="text-[#AAB0D6] text-xs mb-1">Spiritual Experience</div>
                        <p className="text-[#F5F3EE]">{app.spiritualExperience}</p>
                      </div>
                    )}
                    {app.currentPractices && (
                      <div>
                        <div className="text-[#AAB0D6] text-xs mb-1">Current Practices</div>
                        <p className="text-[#F5F3EE]">{app.currentPractices}</p>
                      </div>
                    )}
                    <div>
                      <div className="text-[#AAB0D6] text-xs mb-1">Weekly Availability</div>
                      <p className="text-[#F5F3EE]">{app.availableTimeWeekly}</p>
                    </div>
                  </div>
