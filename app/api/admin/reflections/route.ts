...
// simple surah name map (you can replace with DB later)
...
export async function GET(req: NextRequest) {
  ...
  const formatted = reflections.map((r) => ({
    ...r,
    surahName: surahNames[r.surahNumber] || `Surah ${r.surahNumber}`,
  }));

  return NextResponse.json({ reflections: formatted });
}